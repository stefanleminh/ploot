...
module.exports = {
  ...
  execute (oldState, newState, client) {
    const isLobbyUpdate =
      newState.channelId === client.config.lobby ||
      oldState.channelId === client.config.lobby
    ...
    if (oldUserChannel === null && newState.channelId === client.config.lobby) {
      ...
      if (newState.member.user.bot) {
        logger.debug(
          'Participant ' +
            newState.member.user.username +
            ' is a bot and will not be added.'
        )
        return
      }
      client.currentPlayers.push(newState.member.user)
      client.channels.cache
        .get(client.config.botChannel)
        .send('Added player <@' + newState.member.id + '> as a player.')
      logger.debug(
        'Added participant ' + newState.member.user.username + ' to lobby'
      )
    } else if (
      newUserChannel === null &&
      oldState.channelId === client.config.lobby
      ...
      client.currentPlayers = client.currentPlayers.filter(
        (player) => player !== oldState.member.user
      )
      client.currentSpectators = client.currentSpectators.filter(
        (spectator) => spectator !== oldState.member.user
      )
      client.channels.cache
        .get(client.config.botChannel)
        .send('Removed player <@' + oldState.member.id + '> from lobby.')
      logger.debug(
